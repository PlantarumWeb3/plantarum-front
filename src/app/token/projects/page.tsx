...
export default function ProjectsPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (maturityTimestamp <= now) {
      alert("⚠️ La fecha de vencimiento (maturity) debe ser FUTURA");
      return;
    }
      ...
      >
        <form onSubmit={handleSubmit} className="flex flex-col gap-5" style={{ maxWidth: "900px", margin: "0 auto" }}>
          ...
          <input
            ...
            onChange={handleChange}
            type="date"
            placeholder="📆 Fecha de vencimiento (DEBE ser futura)"
            className="input-dark"
            required
            disabled={!form.price}
          ...
          <button type="submit" className="btn-green w-full mt-4" disabled={!form.phases}>
