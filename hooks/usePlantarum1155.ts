...
export default function usePlantarum1155() {
  ...
  const getContract = async () => {
    if (typeof window !== "undefined" && (window as any).ethereum) {
      // Navegador con MetaMask
      const provider = new ethers.BrowserProvider(
        (window as any).ethereum
      ) as any; // 👈 forzamos a any
      const signer = await provider.getSigner();
      return new ethers.Contract(CONTRACT_ADDRESS, Plantarum1155ABI, signer) as any; // 👈 forzamos a any
    } else {
      // SSR (Vercel) o sin MetaMask → solo lectura
      const provider = new ethers.JsonRpcProvider(
        process.env.NEXT_PUBLIC_RPC_URL
      );
      return new ethers.Contract(CONTRACT_ADDRESS, Plantarum1155ABI, provider) as any; // 👈 forzamos a any
    }
  };
  ...
  const listCarbonForSale = async (tokenId: number, price: string) => {
    const contract = await getContract();
    const tx = await contract.listCarbonForSale(
      tokenId,
      ethers.parseEther(price)
    );
    return tx.wait();
  ...
  const listProjectForSale = async (tokenId: number, price: string) => {
    const contract = await getContract();
    const tx = await contract.listProjectForSale(
      tokenId,
      ethers.parseEther(price)
    );
    return tx.wait();
